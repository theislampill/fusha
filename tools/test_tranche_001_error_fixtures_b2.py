#!/usr/bin/env python3
"""Focused tests for tranche-001 Line 4 hostile error fixtures, batch B2 (morphology_general / clitics_affixes /
script_phonology) — the FINAL batch of the append-only 101-row T1 hostile-fixture trace.

Pins the 34 source queue rows named by this worker's private pointer manifest
(schema private.t1_error_fixture_batch_b2_pointer_manifest.v1) against the COMMITTED public evidence those rows
must reverse-trace to: curriculum/l1l6/reports/queues/q-error-fixtures.jsonl (lesson/domain),
curriculum/l1l6/qualification/*.jsonl (per-lesson qualification), and
curriculum/l1l6/misconceptions/misconception-registry.jsonl (per-misconception manifestation/unit linkage).

This batch continues batch A (tools/test_tranche_001_error_fixtures.py) and batch B1
(tools/test_tranche_001_error_fixtures_b1.py) without touching their truth: batch A's sarf bank
(sarf/evals/tranche-001-error-fixtures-a.jsonl), batch B1's sarf bank
(sarf/evals/tranche-001-error-fixtures-b1.jsonl), and the first 67 rows of the shared trace
(curriculum/l1l6/reports/mistake-pattern-fixtures.jsonl) must stay exactly as committed; only 34 new trace rows
may be appended (101 unique covered queue rows total, zero remaining) and only the shared meta file's counts may
move to record that append. Every new row must be honestly runner_loaded_fixture_only / behaviorally_decided=false
/ not certified — the same disposition batch A and B1 already carry, verified through the SAME real
...
Red-first: this must fail until sarf/evals/tranche-001-error-fixtures-b2.jsonl exists with exactly 34 rows, the
34 matching trace rows are appended, the shared meta counts are updated to 101 loaded / 99 sarf / 2 nahw / 0
decided / 0 remaining, and tools/run_sarf_evals.py registers a fixture_only contract row for the new bank.
"""
...
SARF_BANK_B2 = _REPO / "sarf" / "evals" / "tranche-001-error-fixtures-b2.jsonl"
SARF_BANK_B2_REL = "sarf/evals/tranche-001-error-fixtures-b2.jsonl"

# ---------------------------------------------------------------------------
# the 34 source rows this worker's private pointer manifest names — structural IDs/links only (row ids, lesson
...
# ---------------------------------------------------------------------------
MANIFEST_ROWS = [
    {"source_queue_row_id": "q-error-fixtures-0088", "lesson_id": "L3.M5.04",
     "qualification_path": "curriculum/l1l6/qualification/L3M5.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0008", "mc-0196", "mc-0533", "mc-0817", "mc-0840"]},
    {"source_queue_row_id": "q-error-fixtures-0089", "lesson_id": "L3.M5.05",
     "qualification_path": "curriculum/l1l6/qualification/L3M5.jsonl", "target_domain": "clitics_affixes",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0211", "mc-0498", "mc-0663"]},
    {"source_queue_row_id": "q-error-fixtures-0093", "lesson_id": "L4.M1.03",
     "qualification_path": "curriculum/l1l6/qualification/L4M1.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-s06"],
     "misconception_ids": ["mc-0076", "mc-0225", "mc-0355", "mc-0674", "mc-0899"]},
    {"source_queue_row_id": "q-error-fixtures-0094", "lesson_id": "L4.M1.04",
     "qualification_path": "curriculum/l1l6/qualification/L4M1.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-s01", "u-s06"],
     "misconception_ids": ["mc-0145", "mc-0356", "mc-0653", "mc-0709", "mc-0859"]},
    {"source_queue_row_id": "q-error-fixtures-0097", "lesson_id": "L4.M1.07",
     "qualification_path": "curriculum/l1l6/qualification/L4M1.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0176", "mc-0250", "mc-0423", "mc-0806"]},
    {"source_queue_row_id": "q-error-fixtures-0099", "lesson_id": "L4.M2.02",
     "qualification_path": "curriculum/l1l6/qualification/L4M2.jsonl", "target_domain": "clitics_affixes",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0098", "mc-0296", "mc-0406", "mc-0839", "mc-0844", "mc-0860"]},
    {"source_queue_row_id": "q-error-fixtures-0101", "lesson_id": "L4.M2.04",
     "qualification_path": "curriculum/l1l6/qualification/L4M2.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0223", "mc-0418", "mc-0626", "mc-0826", "mc-0827"]},
    {"source_queue_row_id": "q-error-fixtures-0102", "lesson_id": "L4.M2.05",
     "qualification_path": "curriculum/l1l6/qualification/L4M2.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0040", "mc-0097", "mc-0295", "mc-0376", "mc-0484"]},
    {"source_queue_row_id": "q-error-fixtures-0103", "lesson_id": "L4.M2.06",
     "qualification_path": "curriculum/l1l6/qualification/L4M2.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0070", "mc-0266", "mc-0368", "mc-0441"]},
    {"source_queue_row_id": "q-error-fixtures-0104", "lesson_id": "L4.M2.07",
     "qualification_path": "curriculum/l1l6/qualification/L4M2.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0566", "mc-0568", "mc-0798", "mc-0813", "mc-0815"]},
    {"source_queue_row_id": "q-error-fixtures-0107", "lesson_id": "L4.M3.03",
     "qualification_path": "curriculum/l1l6/qualification/L4M3.jsonl", "target_domain": "script_phonology",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0014", "mc-0202", "mc-0343", "mc-0444", "mc-0483"]},
    {"source_queue_row_id": "q-error-fixtures-0108", "lesson_id": "L4.M3.04",
     "qualification_path": "curriculum/l1l6/qualification/L4M3.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-s06"],
     "misconception_ids": ["mc-0330", "mc-0331", "mc-0499", "mc-0575", "mc-0622", "mc-0795"]},
    {"source_queue_row_id": "q-error-fixtures-0109", "lesson_id": "L4.M3.05",
     "qualification_path": "curriculum/l1l6/qualification/L4M3.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0132", "mc-0280", "mc-0501", "mc-0716"]},
    {"source_queue_row_id": "q-error-fixtures-0114", "lesson_id": "L4.M4.01",
     "qualification_path": "curriculum/l1l6/qualification/L4M4.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0104", "mc-0206", "mc-0283", "mc-0739", "mc-0894"]},
    {"source_queue_row_id": "q-error-fixtures-0115", "lesson_id": "L4.M4.02",
     "qualification_path": "curriculum/l1l6/qualification/L4M4.jsonl", "target_domain": "clitics_affixes",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0005", "mc-0247", "mc-0361", "mc-0362"]},
    {"source_queue_row_id": "q-error-fixtures-0116", "lesson_id": "L4.M4.03",
     "qualification_path": "curriculum/l1l6/qualification/L4M4.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01", "u-s06"],
     "misconception_ids": ["mc-0100", "mc-0249", "mc-0279", "mc-0790", "mc-0889"]},
    {"source_queue_row_id": "q-error-fixtures-0117", "lesson_id": "L4.M4.04",
     "qualification_path": "curriculum/l1l6/qualification/L4M4.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01", "u-s06"],
     "misconception_ids": ["mc-0085", "mc-0152", "mc-0351", "mc-0881", "mc-0892"]},
    {"source_queue_row_id": "q-error-fixtures-0127", "lesson_id": "L4.M5.10",
     "qualification_path": "curriculum/l1l6/qualification/L4M5.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0252", "mc-0562", "mc-0567", "mc-0800"]},
    {"source_queue_row_id": "q-error-fixtures-0130", "lesson_id": "L5.M1.03",
     "qualification_path": "curriculum/l1l6/qualification/L5M1.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0020", "mc-0641", "mc-0688", "mc-0830"]},
    {"source_queue_row_id": "q-error-fixtures-0133", "lesson_id": "L5.M1.06",
     "qualification_path": "curriculum/l1l6/qualification/L5M1.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0049", "mc-0470", "mc-0831"]},
    {"source_queue_row_id": "q-error-fixtures-0134", "lesson_id": "L5.M1.07",
     "qualification_path": "curriculum/l1l6/qualification/L5M1.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0048", "mc-0184", "mc-0625", "mc-0752"]},
    {"source_queue_row_id": "q-error-fixtures-0145", "lesson_id": "L5.M3.04",
     "qualification_path": "curriculum/l1l6/qualification/L5M3.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0241", "mc-0273", "mc-0459", "mc-0723"]},
    {"source_queue_row_id": "q-error-fixtures-0150", "lesson_id": "L5.M4.01",
     "qualification_path": "curriculum/l1l6/qualification/L5M4.jsonl", "target_domain": "clitics_affixes",
     "selected_unit_links": ["u-s06"],
     "misconception_ids": ["mc-0094", "mc-0162", "mc-0775", "mc-0872"]},
    {"source_queue_row_id": "q-error-fixtures-0156", "lesson_id": "L5.M4.07",
     "qualification_path": "curriculum/l1l6/qualification/L5M4.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01", "u-s06"],
     "misconception_ids": ["mc-0112", "mc-0358", "mc-0359", "mc-0600", "mc-0838"]},
    {"source_queue_row_id": "q-error-fixtures-0162", "lesson_id": "L5.M5.06",
     "qualification_path": "curriculum/l1l6/qualification/L5M5.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01", "u-s06"],
     "misconception_ids": ["mc-0029", "mc-0124", "mc-0272", "mc-0290", "mc-0855"]},
    {"source_queue_row_id": "q-error-fixtures-0173", "lesson_id": "L6.M1.06",
     "qualification_path": "curriculum/l1l6/qualification/L6M1.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0313", "mc-0518", "mc-0519", "mc-0553", "mc-0707"]},
    {"source_queue_row_id": "q-error-fixtures-0174", "lesson_id": "L6.M2.01",
     "qualification_path": "curriculum/l1l6/qualification/L6M2.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0071", "mc-0382", "mc-0665", "mc-0734", "mc-0789"]},
    {"source_queue_row_id": "q-error-fixtures-0175", "lesson_id": "L6.M2.02",
     "qualification_path": "curriculum/l1l6/qualification/L6M2.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0007", "mc-0063", "mc-0217", "mc-0271", "mc-0472"]},
    {"source_queue_row_id": "q-error-fixtures-0176", "lesson_id": "L6.M2.03",
     "qualification_path": "curriculum/l1l6/qualification/L6M2.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-s06"],
     "misconception_ids": ["mc-0080", "mc-0613", "mc-0676", "mc-0702", "mc-0890"]},
    {"source_queue_row_id": "q-error-fixtures-0179", "lesson_id": "L6.M3.01",
     "qualification_path": "curriculum/l1l6/qualification/L6M3.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-s06"],
     "misconception_ids": ["mc-0035", "mc-0424", "mc-0656", "mc-0880"]},
    {"source_queue_row_id": "q-error-fixtures-0180", "lesson_id": "L6.M3.02",
     "qualification_path": "curriculum/l1l6/qualification/L6M3.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0126", "mc-0425", "mc-0664", "mc-0749"]},
    {"source_queue_row_id": "q-error-fixtures-0184", "lesson_id": "L6.M3.06",
     "qualification_path": "curriculum/l1l6/qualification/L6M3.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0115", "mc-0464", "mc-0521", "mc-0526", "mc-0691"]},
    {"source_queue_row_id": "q-error-fixtures-0193", "lesson_id": "L6.M5.01",
     "qualification_path": "curriculum/l1l6/qualification/L6M5.jsonl", "target_domain": "clitics_affixes",
     "selected_unit_links": ["u-s01", "u-s06"],
     "misconception_ids": ["mc-0037", "mc-0218", "mc-0219", "mc-0221", "mc-0609"]},
    {"source_queue_row_id": "q-error-fixtures-0204", "lesson_id": "L6.M6.04",
     "qualification_path": "curriculum/l1l6/qualification/L6M6.jsonl", "target_domain": "morphology_general",
     "selected_unit_links": ["u-n01"],
     "misconception_ids": ["mc-0370", "mc-0440", "mc-0446", "mc-0466", "mc-0818"]},
]
...
# ---------------------------------------------------------------------------
# batch A's committed 50-row trace prefix + batch B1's 17-row continuation, compactly pinned (row_id,
# source_queue_row_id, source_lesson, runner_flag, fixture_id, misconception_ids_checked) — "s"=sarf runner/bank,
# "n"=naḥw runner/bank. Used to prove batches A and B1 are never mutated when the B2 rows are appended, and
# cross-checked directly against the live sarf/naḥw bank files (never a duplicated content snapshot of those
# banks).
# ---------------------------------------------------------------------------
_RUNNER = {
    ...
    "s1": ("tools/run_sarf_evals.py", "sarf/evals/tranche-001-error-fixtures-b1.jsonl"),
}
...
EXPECTED_BATCH_B1_TRACE_ROWS = [
    ("trf-b1-trace-0001", "q-error-fixtures-0091", "L4.M1.01", "s1", "trf-b1-sarf-0001",
     ("mc-0119", "mc-0129", "mc-0242", "mc-0592")),
    ("trf-b1-trace-0002", "q-error-fixtures-0092", "L4.M1.02", "s1", "trf-b1-sarf-0002",
     ("mc-0234", "mc-0278", "mc-0353", "mc-0891")),
    ("trf-b1-trace-0003", "q-error-fixtures-0095", "L4.M1.05", "s1", "trf-b1-sarf-0003",
     ("mc-0173", "mc-0278", "mc-0354", "mc-0654", "mc-0898")),
    ("trf-b1-trace-0004", "q-error-fixtures-0096", "L4.M1.06", "s1", "trf-b1-sarf-0004",
     ("mc-0031", "mc-0251", "mc-0348", "mc-0398", "mc-0422")),
    ("trf-b1-trace-0005", "q-error-fixtures-0098", "L4.M2.01", "s1", "trf-b1-sarf-0005",
     ("mc-0056", "mc-0322", "mc-0378", "mc-0512", "mc-0563")),
    ("trf-b1-trace-0006", "q-error-fixtures-0111", "L4.M3.07", "s1", "trf-b1-sarf-0006",
     ("mc-0193", "mc-0257", "mc-0435", "mc-0436", "mc-0904")),
    ("trf-b1-trace-0007", "q-error-fixtures-0151", "L5.M4.02", "s1", "trf-b1-sarf-0007",
     ("mc-0039", "mc-0245", "mc-0612", "mc-0712")),
    ("trf-b1-trace-0008", "q-error-fixtures-0153", "L5.M4.04", "s1", "trf-b1-sarf-0008",
     ("mc-0061", "mc-0182", "mc-0208", "mc-0666")),
    ("trf-b1-trace-0009", "q-error-fixtures-0154", "L5.M4.05", "s1", "trf-b1-sarf-0009",
     ("mc-0117", "mc-0364", "mc-0773", "mc-0782")),
    ("trf-b1-trace-0010", "q-error-fixtures-0155", "L5.M4.06", "s1", "trf-b1-sarf-0010",
     ("mc-0013", "mc-0212", "mc-0300", "mc-0492")),
    ("trf-b1-trace-0011", "q-error-fixtures-0158", "L5.M5.02", "s1", "trf-b1-sarf-0011",
     ("mc-0171", "mc-0433", "mc-0473", "mc-0630")),
    ("trf-b1-trace-0012", "q-error-fixtures-0181", "L6.M3.03", "s1", "trf-b1-sarf-0012",
     ("mc-0022", "mc-0337", "mc-0389", "mc-0801")),
    ("trf-b1-trace-0013", "q-error-fixtures-0185", "L6.M3.07", "s1", "trf-b1-sarf-0013",
     ("mc-0452", "mc-0506", "mc-0574", "mc-0794", "mc-0887")),
    ("trf-b1-trace-0014", "q-error-fixtures-0192", "L6.M4.06", "s1", "trf-b1-sarf-0014",
     ("mc-0006", "mc-0052", "mc-0591", "mc-0763")),
    ("trf-b1-trace-0015", "q-error-fixtures-0194", "L6.M5.02", "s1", "trf-b1-sarf-0015",
     ("mc-0188", "mc-0244", "mc-0500", "mc-0743", "mc-0888")),
    ("trf-b1-trace-0016", "q-error-fixtures-0197", "L6.M5.05", "s1", "trf-b1-sarf-0016",
     ("mc-0030", "mc-0073", "mc-0175", "mc-0699")),
    ("trf-b1-trace-0017", "q-error-fixtures-0203", "L6.M6.03", "s1", "trf-b1-sarf-0017",
     ("mc-0064", "mc-0179", "mc-0615", "mc-0756", "mc-0832")),
]
...
FORBIDDEN_LEAKAGE_SUBSTRINGS = ("_private", "pointer-manifest", "source_manifest", "batch-b2-pointer-manifest",
                                "qualification_path", "expected_decision", "qamus_source_card", ":\\\\", "C:\\\\")
B2_TRACE_ROW_ID_RE = re.compile(r"^trf-b2-trace-\d{4}$")
B2_FIXTURE_ID_RE = re.compile(r"^trf-b2-sarf-\d{4}$")
...
class ManifestSourceTruthTests(unittest.TestCase):
    """The 34 pinned rows are independently re-verified against COMMITTED public evidence, never trusted from
    the private manifest alone."""

    def test_manifest_pins_exactly_34_unique_rows(self):
        self.assertEqual(len(MANIFEST_ROWS), 34)
        self.assertEqual(len(set(MANIFEST_ROW_IDS)), 34)
    ...
    def test_every_misconception_id_exists_with_matching_lesson_manifestation_and_unit_link(self):
        ...
        for row in MANIFEST_ROWS:
            self.assertGreaterEqual(len(row["misconception_ids"]), 3)
    ...
    def test_no_overlap_with_batch_a_or_b1_selected_rows(self):
        batch_a_ids = {t[1] for t in EXPECTED_BATCH_A_TRACE_PREFIX}
        batch_b1_ids = {t[1] for t in EXPECTED_BATCH_B1_TRACE_ROWS}
        self.assertEqual(batch_a_ids & set(MANIFEST_ROW_IDS), set())
        self.assertEqual(batch_b1_ids & set(MANIFEST_ROW_IDS), set())

    def test_manifest_rows_are_absent_from_the_final_remaining_set(self):
        meta = json.loads(TRACE_META.read_text(encoding="utf-8"))
        self.assertEqual(set(meta["remaining_row_ids"]) & set(MANIFEST_ROW_IDS), set())


class PriorBatchImmutabilityTests(unittest.TestCase):
    """Batch A's and batch B1's own trace rows and bank files must never move when B2 is appended."""

    def test_trace_prefix_67_rows_are_byte_identical_to_the_pinned_a_plus_b1_snapshot(self):
        trace = _jsonl(TRACE)
        self.assertGreaterEqual(len(trace), 67)
        prefix = trace[:67]
        expected = EXPECTED_BATCH_A_TRACE_PREFIX + EXPECTED_BATCH_B1_TRACE_ROWS
        for got, expected_row in zip(prefix, expected):
            runner, bank = _RUNNER[expected_row[3]]
            self.assertEqual(got["row_id"], expected_row[0])
            self.assertEqual(got["source_queue_row_id"], expected_row[1])
            self.assertEqual(got["source_lesson"], expected_row[2])
            self.assertEqual(got["owning_runner"], runner)
            self.assertEqual(got["owning_skill_bank"], bank)
            self.assertEqual(got["fixture_id"], expected_row[4])
            self.assertEqual(tuple(got["misconception_ids_checked"]), expected_row[5])
    ...
    def test_sarf_bank_b1_still_has_17_rows_matching_the_pinned_trace_cross_links(self):
        by_id = {r["id"]: r for r in _jsonl(SARF_BANK_B1)}
        self.assertEqual(len(by_id), 17)
        for row_id, source_row_id, source_lesson, flag, fixture_id, mc_ids in EXPECTED_BATCH_B1_TRACE_ROWS:
            bank_row = by_id.get(fixture_id)
            self.assertIsNotNone(bank_row, "%s missing from batch B1 sarf bank" % fixture_id)
            self.assertEqual(bank_row["source_row_id"], source_row_id)
            self.assertEqual(bank_row["source_lesson"], source_lesson)
            self.assertEqual(sorted(bank_row["misconception_ids"]), sorted(mc_ids))
...
class TraceAppendTests(unittest.TestCase):
    """34 new rows appended after batch A+B1's 67, for 101 unique covered queue rows total, zero remaining."""

    def test_trace_has_exactly_101_rows(self):
        self.assertEqual(len(_jsonl(TRACE)), 101)

    def test_appended_rows_are_a_bijection_with_the_manifest(self):
        trace = _jsonl(TRACE)
        appended = trace[67:101]
        self.assertEqual(len(appended), 34)
        ...
        for rid in row_ids:
            self.assertRegex(rid, B2_TRACE_ROW_ID_RE)

    def test_appended_rows_carry_exact_reverse_links_to_the_manifest(self):
        trace = _jsonl(TRACE)
        appended = {r["source_queue_row_id"]: r for r in trace[67:101]}
        for row in MANIFEST_ROWS:
            ...
            self.assertEqual(got["owning_skill_bank"], SARF_BANK_B2_REL)
            self.assertRegex(got["fixture_id"], B2_FIXTURE_ID_RE)
    ...
    def test_appended_rows_leak_no_private_source_or_answer_key(self):
        for row in _jsonl(TRACE)[67:101]:
            blob = _blob(row)
...
class MetaUpdateTests(unittest.TestCase):
    """The shared meta file honestly records the append; behaviorally_decided stays zero throughout; the
    candidate universe reaches zero remaining."""

    def test_meta_counts_reflect_101_loaded_rows_zero_decided(self):
        meta = json.loads(TRACE_META.read_text(encoding="utf-8"))
        self.assertEqual(meta["outcomes"]["runner_loaded_fixture_only_count"], 101)
        ...
        self.assertEqual(meta["domain_routing"]["sarf_runner_loaded_count"], 99)
    ...
    def test_meta_remaining_count_drops_to_zero(self):
        meta = json.loads(TRACE_META.read_text(encoding="utf-8"))
        self.assertEqual(meta["remaining_row_count"], 0)
...
class BankFixtureTests(unittest.TestCase):
    """34 original hostile fixtures, one per manifest row, structurally hostile and never a promotion surface."""

    def test_bank_has_exactly_34_rows_one_per_manifest_row(self):
        rows = _jsonl(SARF_BANK_B2)
        self.assertEqual(len(rows), 34)
        ids = [r["id"] for r in rows]
        self.assertEqual(len(ids), len(set(ids)))
        for rid in ids:
            self.assertRegex(rid, B2_FIXTURE_ID_RE)
        batch_a_ids = {r["id"] for r in _jsonl(SARF_BANK_A)}
        batch_b1_ids = {r["id"] for r in _jsonl(SARF_BANK_B1)}
        self.assertEqual(set(ids) & batch_a_ids, set())
        self.assertEqual(set(ids) & batch_b1_ids, set())
    ...
    def test_every_row_carries_exact_lesson_domain_and_misconception_reverse_links(self):
        rows = {r["source_row_id"]: r for r in _jsonl(SARF_BANK_B2)}
        for row in MANIFEST_ROWS:
    ...
    def test_hostile_positive_negative_discrimination_is_meaningful(self):
        seen_boundaries = set()
        for row in _jsonl(SARF_BANK_B2):
...
class RunnerLoadTests(unittest.TestCase):
    ...
    def test_b2_bank_is_registered_fixture_only_and_reports_zero_decided_rows(self):
        import run_sarf_evals as rse
        contract = rse.load_contract(str(_REPO))
        spec = rse.bank_spec(contract, SARF_BANK_B2_REL)
        ...
        my_errors = [e for e in errors if "tranche-001-error-fixtures-b2" in e]
    ...
    def test_b2_bank_resolves_by_the_basename_cli_form(self):
        import run_sarf_evals as rse
        contract = rse.load_contract(str(_REPO))
        spec = rse.bank_spec(contract, "tranche-001-error-fixtures-b2.jsonl")
        self.assertEqual(spec["path"], SARF_BANK_B2_REL)
        with self.assertRaises(KeyError):
            rse.bank_spec(contract, "tranche-001-error-fixtures-b2")
