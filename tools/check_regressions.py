...
for _art in (
        ...
        "qamus/examples/two_vote_artifact.sample.jsonl",
...
for _script, _args, _label in (
        ...
        ("test_fusha_bench.py", [],
         "FUSHA-BENCH v1 focused authority/runtime tests pass"),
        ("fusha_bench.py", ["--self-test"],
         "FUSHA-BENCH v1 hostile mutation suite passes"),
        ("fusha_bench.py", ["--check"],
         "FUSHA-BENCH v1 committed baseline is byte-identical to recompute"),
        ...
        ("validate_two_vote_artifacts.py", ["--self-test"],
         "Two-vote artifact contract validator self-test (independent vote pair; conclusion+reason agreement; reclassification state)"),
        ("validate_two_vote_artifacts.py",
         [os.path.join(_R, "qamus", "examples", "two_vote_artifact.sample.jsonl")],
         "Two-vote artifact sample validates (verified pair + honestly reclassified historical claim)"),
...
try:
    ...
    _ll4b = run_text([sys.executable, os.path.join(ROOT, "tools", "validate_fusha_standalone_parse.py"), "--self-test"])
    check("largelexicon standalone parser self-test (registry fail-closed, rival preservation, exact scoped spans)", _ll4b.returncode == 0)
    ...
    _ll6a = run_text([sys.executable, os.path.join(ROOT, "tools", "project_largelexicon_qamus_hover_candidates.py"), "--self-test"])
    check("largelexicon hover projector self-test (partial-span contiguity and collision withholding)", _ll6a.returncode == 0)
