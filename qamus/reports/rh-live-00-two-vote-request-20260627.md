...
This packet moves only the source-supported rows into the established Phase 4 two-vote request format. It does not
certify, apply, or preview them publicly.

## Artifacts

| artifact | role |
|---|---|
| `qamus/examples/rh_live_00_two_vote_request_from_source_readiness.sample.jsonl` | three exact-address two-vote request rows |
...
The six source-retry rows remain excluded from this two-vote packet and must not be silently promoted.
