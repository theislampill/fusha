...
| M5 | **Common-error diagnosis** — 205 lessons carry a learner-error section (wrong form → why wrong → correction; heading variants: Common Mistakes / … to Avoid / Common Errors to Avoid / Common Confusion Points, matched semantically) | 205 mistake sections (`registry/lessons.jsonl` counts) | `curriculum/drills/dogfood-error-remediation-index.md`, `tools/fusha_learner_feedback.py` (KC violation records) | direct feed: restated mistake patterns become learner-error KC entries and adversarial fixtures |
