...
| 8 | **Fact-level certification** — the certifier transitions `candidate → review_required → certified` (or `blocked`/`rejected`) per fact against the evidence ladder; append-only hash-chained event trail; it *refuses* facts whose bundles fall short | `tools/certify_typed_fact.py` (`--validate`, `--self-test`, `--demo-sufaha`); `certification/events.jsonl` (147 events) |
