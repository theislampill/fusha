...
**Update 2026-07-30 (Burst A3, defect rounds 1-2):** §9 and §10 add an executed target-schema promotion and an executable typed bridge. **Neither GAP-L1 nor GAP-L3 is closed.** GAP-L1 remains open while harness integration of the deterministic promotion gates is still owed to the serialized integration lane. GAP-L3 remains open because the public repository contains no canonical typed lexeme/form graph bundle at usable scale: only 2 eligible edges at 1 occurrence loc exist, and the bridge therefore produces **zero** candidates on real data. `qamus/task-packets/TP-LARGELEXICON-A3-BRIDGE-WAVE1.json` packetizes exactly that missing dependency. The bridge path is real, adversarially tested and fail-closed; what is missing is its evidence, not its code.
...
**Abstention vocabulary** (closed, **fifteen** blockers, each with a projection status): `lexical_collision_requires_context`, `page_context_only_no_lexical_edge`, `root_family_relation_not_lexeme_identity`, `non_certified_graph_edge_only`, `typed_edge_identity_disagreement`, `missing_typed_graph_evidence`, `norm_only_surface_match`, `unresolved_canonical_loc`, `loc_not_in_canonical_index`, `loc_surface_disagreement`, `wbw_loc_disagreement`, `crosswalk_packet_not_accepted`, `quarantined_or_flagged_source_row`, `missing_dependency_release`, `no_carried_lexical_support`. Support requires an **exact written-surface** match; a normalized-only match is a `never_auto_resolve` trigger in the gate SSOT and stays blocked. Reuse is loc-first: `missing-loc|…` and `sarf:surface:…` are diagnostic labels and are rejected as binding keys. Zero, one and many candidates are all preserved as-is; a multi-entry collision keeps every candidate, records an unresolved `tension_record`, and never selects a winner.
